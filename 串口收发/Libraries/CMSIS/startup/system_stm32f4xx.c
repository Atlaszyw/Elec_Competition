...
/*
#:
用户可以调用SystemCoreClockUpdate()函数来更新SystemCoreClock变量，
SystemCoreClockUpdate()函数实际上就是根据HSE的值以及上边配置过
程中设置的系数重新计算出来一个SystemCoreClock的值，
相当于读取上边配置好寄存器里边的值，再把系统时钟计算一遍。
但是在更新之前，必须正确设置HSE_VALUE这个宏定义的值，一般在stm32f4xx.h文件中会默认为25M，
用户需要根据自己的需要来修改这个值，我这边晶振是8M。所以HSE_VALUE得值修改为8M。
否则会导致计算出来的SystemCoreClock不正确。
*/
/**
