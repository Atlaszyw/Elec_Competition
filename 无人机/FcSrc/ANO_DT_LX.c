...
static void Add_Send_Data( u8 frame_num, u8* _cnt, u8 send_buffer[] )
{
    ...
    switch ( frame_num )
    {
        ...
        case 0xf1:
        {
            ...
            send_buffer[( *_cnt )++] = BYTE1( temp_data );
            // temp_data                = X_velo;
            temp_data                = ano_of.of1_dx;
            send_buffer[( *_cnt )++] = BYTE0( temp_data );
            send_buffer[( *_cnt )++] = BYTE1( temp_data );
            // temp_data                = Y_velo;
            temp_data                = ano_of.of1_dy;
            send_buffer[( *_cnt )++] = BYTE0( temp_data );
            send_buffer[( *_cnt )++] = BYTE1( temp_data );
            temp_data                = Find_GreenPillar_Times;
            send_buffer[( *_cnt )++] = BYTE0( temp_data );
        }
        break;
        case 0xf2:
        {
            temp_data                = Ctrl_Dst.distance;
            send_buffer[( *_cnt )++] = BYTE0( temp_data );
            send_buffer[( *_cnt )++] = BYTE1( temp_data );
            temp_data                = greenline_feature.offset;
            send_buffer[( *_cnt )++] = BYTE0( temp_data );
            send_buffer[( *_cnt )++] = BYTE1( temp_data );
            temp_data                = redline_feature.offset;
            send_buffer[( *_cnt )++] = BYTE0( temp_data );
            send_buffer[( *_cnt )++] = BYTE1( temp_data );
        }
        break;
