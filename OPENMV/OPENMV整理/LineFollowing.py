...
def find_interserct_lines(lines, angle_threshold=(10, 90), window_size=None):
    ...
    for i in range(line_num - 1):
        for j in range(i, line_num):
            ...
            # 判断角度是否在阈值范围内
            if not (angle >= angle_threshold[0]
                    and angle <= angle_threshold[1]):
                continue
            # 判断交点是否在画面内
            if window_size is not None:
                ...
                if intersect_pt is None:
                    ...
                    continue
                x, y = intersect_pt
                if not (x >= 0 and x < win_width and y >= 0
                        and y < win_height):
                    # 交点如果没有在画面中
                    Line.cross_x = 0
                    Line.cross_y = 0
...
def find_blobs_in_rois(img):
    ...
    if (roi_blobs_result['up']['blob_flag']
            and roi_blobs_result['middle']['blob_flag']
        ...
        Line.flag = 3  #右转
            ...
            not roi_blobs_result['down']['blob_flag']
    ) and roi_blobs_result['up']['blob_flag'] and (
            roi_blobs_result['right']['blob_flag']
            or roi_blobs_result['left']['blob_flag']) and roi_blobs_result[
                'left']['blob_flag'] != roi_blobs_result['right']['blob_flag']:
        Line.flag = 1  #左右转后直线
    else:
        Line.flag = 0  #未检测到
    ...
    elif Line.flag == 1 and (roi_blobs_result['down']['blob_flag']
                             and roi_blobs_result['up']['blob_flag']):
...
def LineCheck():
    ...
    # 寻找相交的点 要求满足角度阈值
    find_interserct_lines(lines,
                          angle_threshold=(45, 90),
    ...
    Message.UartSendData(
        Message.LineDataPack(Line.flag, Line.angle, Line.distance,
